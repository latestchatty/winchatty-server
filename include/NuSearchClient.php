...
function nsc_shackApiGetThread($threadId)
{
   /*
   Return:
   array(
   ...
   */
   ...
   #  {"action":"read","id":"######","method":"thread","format":"json",
   #   "data":{"title":null,"url":null,"comments":false,"post_count":0,"last_id":false}}
   if (isset($d['data']) && 
       isset($d['data']['post_count']) && 
       $d['data']['post_count'] == 0)
   {
      return array('exists' => false, 'visible' => false, 'posts' => false);
   }

   # If the whole thread is nuked:
   #  {"action":"read","id":"######","method":"thread","format":"json",
   #   "data":{"title":"LatestChatty","url":"http:\/\/www.shacknews.com\/chatty","comments":false,
   #           "post_count":1,"last_id":false}}
   if (isset($d['data']) && 
       isset($d['data']['url']) && 
       !is_null($d['data']['url']) &&
       isset($d['data']['comments']) &&
       $d['data']['comments'] === false)
   {
      return array('exists' => true, 'visible' => false, 'posts' => false);
   }

   # If this thread is not publicly visible:
   #  {"action":"read","id":"######","method":"thread","format":"json","error":"Forbidden"}
   if (isset($d['error']))
   {
      return array('exists' => true, 'visible' => false, 'posts' => false);
   }

   # In all other cases, a thread will be returned by Shacknews.
   if (!isset($d['data']) || 
       !isset($d['data']['comments']) || 
       is_null($d['data']['comments']) || 
       !is_array($d['data']['comments']) || 
       empty($d['data']['comments']))
   {
      # This should never happen.  If it does, then we have missed a case.
      throw new Exception('Unexpected response: ' . $json);
   }

   # The comments come in a hierarchical form, but each post contains the parent_id
   # so we can just flatten it and throw away the structure information.
   $dComments = nsc_shackApiGetThread_collect($d['data']['comments']);

   # Now convert it to our own format.
   $comments = array_map('nsc_newPostFromShackApi', $dComments);

   # If this specific post is nuked but not the whole thread, the thread is returned but the 
   # requested post isn't present.
   $present = false;
   ...
   # If the thread is from a Shacknews article thread, the "content_type_id" is "2".
   # If the thread is from the chatty, the "content_type_id" is "17".
   $contentTypeId = intval($dComments[0]['content_type_id']);
   if ($contentTypeId != 2 && $contentTypeId != 17)
      return array('exists' => true, 'visible' => false, 'posts' => false);

   return array('exists' => true, 'visible' => true, 'posts' => $comments);
}

function nsc_shackApiGetThread_collect($dComments)
{
   $list = array();
   ...
   foreach ($dComments as $dComment)
   {
      ...
      if (isset($dComment['comments']))
         $subComments = nsc_shackApiGetThread_collect($dComment['comments']);

      $dComment['comments'] = false;
      $list[] = $dComment;
...
function nsc_newPostFromShackApi($sPost)
{
   return array(
      'id' => intval($sPost['id']),
      'thread_id' => intval($sPost['root_id']),
      'parent_id' => intval($sPost['parent_id']),
      'author' => strval($sPost['user']),
      'category' => nsc_shackApiFlagIntToString($sPost['mod_type_id']),
      'date' => nsc_date(strtotime($sPost['post_time'] . ' PST')),
      'body' => strval($sPost['body'])
   );
}

function nsc_shackApiFlagIntToString($sFlag)
{
   switch ($sFlag)
   {
      case 5: return 'ontopic';
      case 2: return 'nws';
      case 3: return 'stupid';
      case 9: return 'political';
